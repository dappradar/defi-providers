...
const eth = {
  ...
  getTokenBalance: async (account, address, timestamp) => {
    if (address.toLowerCase() == 'hbar') {
      return await eth.getBalance(account, timestamp);
    }
    ...
    get methods() {
      return {
        totalSupply: () => {
          return {
            call: async (timestamp = null) => {
              const res = await call(`tokens/${this.address}`, {
                timestamp,
              });
        ...
        balanceOf: (account) => {
          return {
            call: async (timestamp = null) => {
              const res = await call(`tokens/${this.address}/balances`, {
                'account.id': account,
                timestamp,
              });
