...
async function getPairReservesPerEach(pInfos, block, web3) {
  ...
  let reserves_result = [];
  for (let start = 0; start < pInfosLength; start += 1000) {
    const end = Math.min(pInfosLength, start + 1000);
...
async function getBalances(block, date, chain, provider, web3) {
  ...
  console.log('V2 Calculation Started');
  ...
  for (let first = 0; first < poolInfoLength; first += bigStep) {
    ...
    console.log(`V2 Getting PairInfo from ${first} to ${last}`);
  ...
  console.timeEnd('V2 Getting PairInfo');
