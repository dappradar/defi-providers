...
async function tvl(block) {
  ...
  try {
    pools = JSON.parse(
      fs.readFileSync("./providers/avalanche_curve/pools.json", "utf8")
    );
  } catch {}
  ...
  if (newProviderPools.length > 0) {
    ...
    fs.writeFile(
      "./providers/avalanche_curve/pools.json",
  ...
  ADDRESS_PROVIDERS.forEach((provider) => {
    Object.values(pools[provider]).forEach((token) => {
  ...
  const balances = {};
  util.sumMultiBalanceOf(balances, underlyingBalances);
