...
const TOKEN_ABI = require("./abi.json");
...
async function tvl(block) {
  ...
  try {
    lpTokens = JSON.parse(
      fs.readFileSync("./providers/avalanche_platypus/pools.json", "utf8")
    );
  } catch {}
  ...
  if (newLPTokens.length > 0) {
    ...
    fs.writeFile(
      "./providers/avalanche_platypus/pools.json",
      JSON.stringify(lpTokens, null, 2),
      "utf8",
      function (err) {
        if (err) {
          console.error(err);
