...
async function getKaiVaultBalances(
  web3: any,
  ...
  const vaultObjs = await web3.getObjects(vaults.map((v) => v.id));

  for (let i = 0; i < vaults.length; i++) {
    const vault = vaults[i];
    const vaultObj = vaultObjs[i];

    let tvlAmount = BigInt(vaultObj.fields.free_balance);

    for (const strategy of vaultObj.fields.strategies.fields.contents) {
      tvlAmount += BigInt(strategy.fields.value.fields.borrowed);
    }

    formatter.merge(balances, vault.tType, Number(tvlAmount).toString());
  }
...
async function tvl(params: ITvlParams): Promise<Partial<ITvlReturn>> {
  ...
  try {
    const kaiBalances = await getKaiVaultBalances(web3);
    balances = formatter.sum([balances, kaiBalances]);
  } catch (error) {
    log.error({
      message: error?.message || '',
      stack: error?.stack || '',
      detail: 'Error fetching Kai vault balances',
      endpoint: 'tvl',
    });
  }
